...
## FunPay Parsers 0.7.0

### Features

- Added `funpayparsers.types.enums.SubcategoryFieldType` enum (`UNKNOWN`, `NUMERIC_RANGE`, `TEXT`, `TEXTAREA`, `SELECT`, `DROPDOWN`, `IMAGES`).
- Added `funpayparsers.types.subcategory_structure.FieldCondition` type.
- Added `funpayparsers.types.subcategory_structure.SubcategoryFieldDef` type.
- Added `funpayparsers.types.subcategory_structure.SubcategoryStructure` type with `from_offer_fields()` classmethod.
- Added `funpayparsers.types.offers.OfferFields.field_schema`: subcategory field schema parsed from `data-fields` JSON attribute.
- `funpayparsers.parsers.offer_fields_parser.OfferFieldsParser` now parses the full subcategory field schema from the lot editor form.
- Added `funpayparsers.types.pages.offer_page.OfferPage.param_images`: full-size image URLs extracted from offer attachments.
- Added `funpayparsers.types.pages.offer_page.OfferPage.structured_fields`: offer fields keyed by FunPay field ID, populated when `SubcategoryStructure` is provided.
- Added `subcategory_structure` option to `funpayparsers.parsers.page_parsers.OfferPageParsingOptions`.
- Added `subcategory_structure`, `subcategory_id`, `subcategory_type` options to `funpayparsers.parsers.offer_previews_parser.OfferPreviewsParsingOptions`.
- `funpayparsers.parsers.page_parsers.ProfilePageParser` now stamps `subcategory_id` and `subcategory_type` onto each `OfferPreview`.
- `funpayparsers.parsers.page_parsers.SubcategoryPageParser` now stamps `subcategory_id` and `subcategory_type` onto each `OfferPreview`.
- `funpayparsers.parsers.page_parsers.MyOffersPageParser` now stamps `subcategory_id` and `subcategory_type` onto each `OfferPreview`.
- Added `funpayparsers.types.offers.OfferPreview.subcategory_id` field.
- Added `funpayparsers.types.offers.OfferPreview.subcategory_type` field.
- Added `funpayparsers.types.offers.OfferPreview.parse_title_fields()` method.
- Added `funpayparsers.types.orders.OrderPreview.subcategory_id` field.
- Added `funpayparsers.types.orders.OrderPreview.subcategory_type` field.
- Added `funpayparsers.types.orders.OrderPreview.quantity` field.
- Added `funpayparsers.types.orders.OrderPreview.recipient` field.
- Added `funpayparsers.types.orders.OrderPreview.parse_title_fields()` method.

### Breaking changes

- `funpayparsers.parsers.offer_previews_parser.OfferPreviewsParser`: `OfferPreview.other_data` keys for subcategory fields are now normalized — `data-f-arena` is stored as `arena` instead of `f-arena`.
- `funpayparsers.parsers.offer_previews_parser.OfferPreviewsParser`: `other_data` and `other_data_names` are only populated when `subcategory_structure` is provided. Without a structure both dicts are empty.
