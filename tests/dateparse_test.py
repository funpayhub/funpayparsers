...
CURR_DATE = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0, tzinfo=ZoneInfo('Europe/Moscow'))
...
short_date_obj = datetime(day=12, month=5, year=2024, hour=0, minute=0, second=0, microsecond=0, tzinfo=ZoneInfo('Europe/Moscow'))
...
current_year_date_objs = [datetime(year=CURR_DATE.year, month=i, day=12, hour=12, minute=20, second=0, microsecond=0, tzinfo=ZoneInfo('Europe/Moscow'))
                          for i in MONTHS.values()]
full_date_objs = [datetime(year=2024, month=i, day=12, hour=12, minute=20, second=0, microsecond=0, tzinfo=ZoneInfo('Europe/Moscow'))
                          for i in MONTHS.values()]


def test_time_str_parsing():
    result = parse_date_string(time_str)
    assert result == time_obj.timestamp()


def test_short_date_str_parsing():
    result = parse_date_string(short_date_str)
    assert result == short_date_obj.timestamp()


def test_today_date_str_parsing():
    for i in today_date_strs:
        result = parse_date_string(i)
        assert result == today_date_obj.timestamp()


def test_yesterday_date_str_parsing():
    for i in yesterday_date_strs:
        result = parse_date_string(i)
        assert result == yesterday_date_obj.timestamp()
