...
class ProfilePageParser(FunPayHTMLObjectParser[ProfilePage, ProfilePageParsingOptions]):
    ...
    def _parse(self):
        ...
        badges = [UserBadgeParser(i.html, options=self.options.user_badge_parsing_options).parse()
        ...
        if offer_divs:
            ...
            for offer_div in offer_divs:
                ...
                id_ = int(url.split('/')[-2])
                offers_objs = OfferPreviewsParser(offer_div.html, options=self.options.offer_previews_parsing_options).parse()
        ...
        return ProfilePage(
            ...
            header=PageHeaderParser(header.html, options=self.options.page_header_parsing_options).parse(),
            app_data=AppDataParser(app_data, options=self.options.app_data_parsing_options).parse(),
            ...
            badge=badge,
            achievements=[AchievementParser(i.html, self.options.achievement_parsing_options).parse() for i in achievements_divs],
            ...
            rating=UserRatingParser(rating_div.html, options=self.options.user_rating_parsing_options).parse() if rating_div else None,
            offers=offers,
            chat=ChatParser(chat_div[0].html, options=self.options.chat_parsing_options).parse() if chat_div else None,
            reviews=ReviewsParser(reviews_div[0].html, options=self.options.reviews_parsing_options).parse() if reviews_div else [],
        )
